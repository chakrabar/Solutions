...
namespace Wikipedia.IntegrationTests
{
    ...
    public class AnswerFinderTests
    {
        ...
        public void App_should_find_best_answer()
        {
            ...
            var question = @"What is the aim of the Quagga Project?";
            var answers = new[]
            {
            ...
            var answerFinder = new AnswerFinder();

            var indexData = new ContentIndexBuilder()
                .Build(paragraph);

            var answer = answerFinder.FindBestAnswer(indexData, question, answers);

            Assert.AreEqual(@"aims to breed zebras that are phenotypically similar to the quagga", answer);
        }
    }
}
